...
The `GraphKnowledgeEngine` (in `kogwistar/engine.py`) serves as the base abstraction for the system. It manages the underlying "knowledge graph" or "graph database".
- **Primitives**: The fundamental units are `Node` and `Edge` (in `kogwistar/models.py`).
...
- **Orchestrator**: The `ConversationOrchestrator` (in `kogwistar/conversation_orchestrator.py`) acts as the **workflow entry point** for conversation interactions. It manages the flow of user turns, retrieval, answer generation, and summarization using `add_conversation_turn_workflow_v2`.
...
## Key Files
- `kogwistar/engine.py`: Defines `GraphKnowledgeEngine`, the base graph abstraction.
- `kogwistar/models.py`: Defines the provenance-heavy `Node`/`Edge` primitives and their `Conversation`/`Workflow` extensions.
- `kogwistar/conversation_orchestrator.py`: The entry point for conversation workflows, coordinating the engine and agents.
