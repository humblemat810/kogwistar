"""Run ADR-015 Phase-3 durable-store evidence against current native source.

The gate deliberately separates fake/memory, persistent SQLite, and live
PostgreSQL/pgvector.  PostgreSQL skips are failures: a skipped live group is
environmental evidence missing, never a passing Phase-3 gate.
"""
...
DEFAULT_REPORT = ROOT / ".codex" / "adr015-phase3-store-gate.json"

MEMORY_TESTS = (
    "tests/core/test_rust_store_memory_differential.py",
)
SQLITE_TESTS = (
    "tests/core/test_rust_store_sqlite_differential.py",
    "tests/core/test_rust_store_projection_differential.py",
    "tests/core/test_rust_store_index_jobs_differential.py",
    "tests/core/test_rust_store_lane_messages_differential.py",
    "tests/core/test_rust_store_recovery_differential.py",
    "tests/core/test_rust_store_run_registry_differential.py",
    "tests/core/test_rust_store_workflow_history_differential.py",
)
POSTGRES_TESTS = (
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Repeat to run selected groups; default runs all three.",
    )
...
def _load_native_report(path: Path, source_sha256: str) -> dict[str, Any]:
    ...
    if payload.get("schema") != "adr015-host-native/v1":
...
def _run_group(
    *,
    ...
    command = [str(python), "-m", "pytest", "-q", f"--junitxml={junit}"]
    if live_postgres:
        command.extend(("-k", "postgres"))
...
def run_gate(
    *,
    ...
    specifications = {
        ...
        "sqlite": (SQLITE_TESTS, False),
        "postgres": (POSTGRES_TESTS, True),
    }
    reports = [
        _run_group(
            python=python,
            name=name,
            tests=specifications[name][0],
            output_root=output_root,
            live_postgres=specifications[name][1],
        )
        for name in groups
    ]
    return {
        "schema": "adr015-phase3-store-gate/v1",
...
def main() -> int:
    ...
    selected = tuple(args.group or ("memory", "sqlite", "postgres"))
    report_path = args.report.expanduser().resolve()
    result = run_gate(
        python=python,
        native_report=args.native_report.expanduser().resolve(),
        output_root=report_path.parent / "adr015-phase3-store-gate",
        groups=selected,
    )
