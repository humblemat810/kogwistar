...
def _spawn_worker(*, python_exe: str, worker_module: str, args: list[str]) -> subprocess.Popen:
...
def _main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Cross-platform supervisor for the index job worker process")
    ...
    parser.add_argument("--pidfile", default="", help="Write worker PID here (for tests/ops)")
    parser.add_argument("--restart-delay-ms", type=int, default=100, help="Delay before restarting worker after exit")
    parser.add_argument("--phase1-enable-index-jobs", action="store_true")
    parser.add_argument("--worker-module", default="graph_knowledge_engine.workers.index_job_worker", help="Module path to run as worker")
    ...
    worker_args = [
        "--persist-directory", args.persist_directory,
        "--namespace", args.namespace,
        "--tick-interval-ms", str(args.tick_interval_ms),
        "--batch-size", str(args.batch_size),
        "--max-jobs-per-tick", str(args.max_jobs_per_tick),
    ...
    while not stop["flag"]:
        if proc is None or proc.poll() is not None:
            # (re)start worker
            proc = _spawn_worker(python_exe=python_exe, worker_module=args.worker_module, args=worker_args)
