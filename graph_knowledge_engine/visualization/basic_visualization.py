    ...
    def resolve_readable(
        ...
        *,
        ...
        if by_doc_id:
            ...
            n_links = self.e.node_docs_collection.get(where={"doc_id": by_doc_id}, include=["documents"])
            ...
            e_links = self.e.edge_endpoints_collection.get(where={"doc_id": by_doc_id}, include=["documents"])
            edge_ids = list({
        ...
        node_out = []
        if node_map:
            got = self.e.node_collection.get(ids=list(node_map.keys()), include=["metadatas", "documents"])
        ...
        edge_out = []
        if edge_map:
            got = self.e.edge_collection.get(ids=list(edge_map.keys()), include=["metadatas", "documents"])
