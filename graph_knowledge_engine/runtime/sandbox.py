...
class DockerPythonSandbox(Sandbox):
    ...
    def _make_container_name(self, run_id: str) -> str:
        safe = "".join(ch if ch.isalnum() else "-" for ch in run_id).strip("-").lower() or "run"
    ...
    def _remove_container(self, name: Optional[str], *, run_id: Optional[str] = None) -> None:
        if not name:
            return
        if run_id:
            with self._lock:
    ...
    def _ensure_run_container(self, run_id: str) -> str:
        with self._lock:
            ...
            try:
                ...
                raise RuntimeError(f"Timed out starting sandbox container after {self.timeout_s:.0f}s") from exc
            if proc.returncode != 0:
                self._remove_container(name)
                stderr = (proc.stderr or proc.stdout or "").strip()
                raise RuntimeError(f"Failed to start sandbox container: {stderr or 'unknown error'}")
            ...
            if self.mode == "per_run":
                ...
                if not run_id:
                    return RunFailure(
                        conversation_node_id=None,
                        state_update=[],
                        errors=["Container sandbox in per_run mode requires context['run_id']"],
                    )
                container_name = self._ensure_run_container(run_id)
                cmd = [self.runtime_cmd, "exec", "-i", container_name, self.python_bin, "-c", runner]
            else:
                container_name = self._make_container_name(str(context.get("run_id") or "op"))
                cmd = [
                    self.runtime_cmd,
                    "run",
                    "--rm",
    ...
    def create(sandbox_type: str, config: Dict[str, Any]) -> Sandbox:
        if sandbox_type == "local":
            ...
            return DockerPythonSandbox(
                ...
                container_name_prefix=config.get("container_name_prefix", "gke-sandbox"),
