...
def regen_doc(folder_path, use_raw=False):
    pages_nums = sorted(
        (
            ...
            if i.endswith(".json") and i.startswith("page_")
        )
    )
    pages = []
    split_pages = []
...
class RawOCRResponse(BaseModel):
    ...
    boxes_2d: list[box_2d] = Field(
        description="description of x min, y min, xmax and y max. Share id uniqueness with signature blocks. "
    )
    ...
    def check_cluster_meaningful_ordering_agreement(self):
        ...
        if not len([i.id for i in (self.non_text_objects + self.boxes_2d)]) == len(
            set(i.id for i in self.non_text_objects + self.boxes_2d)
        ):
            raise ValueError(
...
class OCRClusterResponseMetaless(ModeSlicingMixin, BaseModel):
    ...
    OCR_text_clusters: DtoType[list[TextCluster]] = Field(
        description="the OCR text results."
    )
...
class RawOCRResponseMetaless(ModeSlicingMixin, BaseModel):
    boxes_2d: list[box_2d] = Field(
        description="description of x min, y min, xmax and y max"
    )
    non_text_objects: DtoType[list[NonText_box_2d]] = Field(
        description="the non-OCR object results. Share cluster number uniqueness with OCR texts. "
    )
...
def get_first_round_response(
    ...
    if (response is None) or parsing_error:
        ...
        if (
            ...
            and (ocr_draft_response.text is not None)
            and ocr_draft_response.text != ""
        ):
            draft_responses[model_name] = ocr_draft_response.text
        sys_message_2.content += (
            "If your internal OCR fails. Focus on table parsing mode because my error analysis modes often show that the failing OCR pages are usually highly complicated tables. "
            f"Try to put in as much data as possible given all text found by simple OCR for your reference:```{ocr_draft_response.text}```"
            if ocr_draft_response
            else ""
        )
...
def validate_response(
    ...
    if response is None:
        logger.error(
            f"LLM returned None as response, file name = {image_file_path}, {model_name=}"
        )
        raise (
            ValueError(
                f"LLM returned None as response, file name = {image_file_path}, {model_name=}"
        ...
        if (
            len(response.OCR_text_clusters) == 0
            or len("".join([c.text for c in response.OCR_text_clusters])) == 0
        ):
            logger.info(f"emptydoc by {model_name}")
            if (
                model_name != "gemini-2.5-pro"
            ):  # only trust the verdict from newest advanced model if nothing detected.
                raise Exception(
                    f"Empty OCR Page error. No text detected at all by less advanced model {model_name}. "
        ...
        response_dict_local["pdf_page_num"] = page_file_name.rsplit(".", 1)[0].rsplit(
            "_", 1
        )[-1]
        response_dict_local["metadata"] = {
            "ocr_model_name": model_name,
            "ocr_datetime": time.time(),
            "ocr_json_version": str(ocr_json_version),
        }
...
class NonTextObject(BaseModel):
    ...
    bounding_box: list[int] = Field(
        description="Bounding box of identified non-OCR object"
    )
    id: int = Field(description="id of the text box in the page, autoincrement from 0")
...
class TextBoxResponse(BaseModel):
    ...
    non_text_blocks: list[NonTextObject] = Field(
        description="bounding boxes and description of the object identified"
    )
    meaningful_ordering: DtoType[list[int]] = Field(
        description="The correct meaningful ordering of the identified text clusters. Must cover all OCR_text_clusters once and only once. "
    )
...
def RawOCRResponse_to_OCRClusterResponse(
    raw_response: RawOCRResponse | RawOCRResponseMetaless | TextBoxResponse,
) -> OCRClusterResponse:
    temp = raw_response.model_dump()
    boxes_2d = temp.pop("boxes_2d")  # y min x min, y max x max
    temp["OCR_text_clusters"] = [
        TextCluster.model_validate(
            {
                "text": i["label"],
                "bb_y_min": i["box_2d"][0],
                "bb_x_min": i["box_2d"][1],
    ...
    temp["non_text_objects"] = [
        NonTextCluster.model_validate(
            {
                "description": i["label"],
                "bb_y_min": i["box_2d"][0],
                "bb_x_min": i["box_2d"][1],
...
def final_resort(
    ...
    earlier_partial_ocr = (
        ...
        or draft_responses.get("gemini-2.5-flash")
        or max_v
    )
    ...
    if ocr_meta_response is None:
        raise Exception(
            "model capability cannot even skim meta coarse level information"
        )
    metadata = [
        SystemMessage(
            "Earlier steps has already determined the metadata about this document: \n\n"
    ...
    if has_error:
        ...
        try:
            ...
            try:
                ...
                response_dict["pdf_page_num"] = page_file_name.rsplit(".", 1)[0].rsplit(
                    "_", 1
                )[-1]
...
def TextBoxResponse_to_OCRClusterResponse(
    raw_response: TextBoxResponse, meta_response: OCRMetaResponse
) -> OCRClusterResponse:
...
def refine_image_response(
    ...
    if response_dict.get("refined_pipeline_run"):
        return False
    if (
        not retry_failed_refine
        and response_dict.get("refined_pipeline_failed_reason") is not None
    ):
        return False
        ...
        try:
            ...
            if "flash-lite" in model_name:
                ...
                if i_model >= min(len(model_names), 20):
                    logger.error(
                        f"All LLM returned None as response, file name = {image_file_path}"
                    )
                    raise (
                        ValueError(
                            f"All LLM returned None as response, file name = {image_file_path}"
            ...
            if refined:
                response_dict["refined_version"]["usage_metadata"] = (
                    cb.model_dump()
                )  # or usage_metadata
...
def get_messages(image_file_path):
    ...
    sys_message = SystemMessage(
        "You are a helpful raw document AI that does OCR (Optical Character Reading) that focus on extracting raw document meta. "
...
def ocr_single_image(
    ...
    with get_gemini_callback_cost() as cb:
        if (
            ...
            and exist_behavior == "rerun"
            or not os.path.exists(outfile_name)
        ):
        ...
        if allow_page_refine:
            refined = refine_image_response(
                ok2, response_dict, outfile_name, image_file_path, model_names, cb
            )
            if refined:
                time.sleep(5)
...
def refine_table_ocr(response_dict, llm: BaseChatModel, cb, error_messages):
    ...
    system_prompt = SystemMessage(
        "You are an OCR data organiser. Your job is to refine the user query that contains existing OCR raw text clusters to become meaningful. \n"
    ...
    for i in range(max_attempt):
        try:
            ...
            is_preserved = [
                fuzz.partial_ratio(text_after, i) >= get_threshold(i)
                for i in text_before
            ]
            lost_text = [
                i
                for i, preserved in zip(sp.OCR_text_clusters, is_preserved)
                if not preserved
            ]
            ok = all(
                tf or (tc.text.strip() == "")
                for tf, tc in zip(is_preserved, sp.OCR_text_clusters)
            )
            assert ok, (
                f"Some text or punctuations are lost through OCR text grouping, lost text = {str(lost_text)}"
            )
