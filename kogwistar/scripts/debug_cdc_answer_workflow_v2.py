...
(using kogwistar.utils.kge_debug_dump.dump_d3_bundle), capture WS stream,
...
if TYPE_CHECKING:
    from kogwistar.engine_core.engine import EngineType
import time
import webbrowser
...
def dump_bundle(
    *, template_path: Path, out_html: Path, engine_type: EngineType, ws_url: str
    ...
    try:
        from kogwistar.utils.kge_debug_dump import dump_d3_bundle  # type: ignore
    except Exception as e:
        raise RuntimeError(
            "Failed to import kogwistar.utils.kge_debug_dump.dump_d3_bundle. "
...
def main() -> int:
    ...
    ap.add_argument(
        "--template",
        default="kogwistar/templates/d3.html",
