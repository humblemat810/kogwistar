...
def build_engine(*, persist_dir: Path, graph_type: _GRAPH_TYPE) -> GraphKnowledgeEngine:
    ...
    return GraphKnowledgeEngine(
        persist_directory=str(persist_dir), kg_graph_type=graph_type
    )
...
def _render_template_html(template_html: str, *, context: dict) -> str:
    ...
    if "{{" in html or "{%" in html:
        raise RuntimeError(
            "Rendered bundle still contains Jinja tokens ('{{' or '{%')."
        )
    return html
    ...
    *,
    ...
    payload = (
        ...
        if embed_empty
            ...
            engine, doc_id=doc_id, insertion_method=insertion_method, mode=mode
        )
    )
...
def _cmd_pair(args: argparse.Namespace) -> None:
    ...
    if not args.empty:
        kg_engine = build_engine(
            persist_dir=Path(args.kg_persist_dir), graph_type="knowledge"
        )
        conv_engine = build_engine(
            persist_dir=Path(args.conversation_persist_dir), graph_type="conversation"
        )
        workflow_engine = build_engine(
            persist_dir=Path(args.workflow_persist_dir), graph_type="workflow"
        )
    else:
        kg_engine = None
        conv_engine = None
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Dump self-contained D3 debug bundles for GraphKnowledgeEngine"
    )
    ...
    p1.add_argument(
        "--graph-type",
        choices=["knowledge", "conversation", "workflow"],
        default="knowledge",
        help="Which graph",
    )
    p1.add_argument("--doc-id", help="Optional doc_id scope")
    p1.add_argument(
        "--mode", default="reify", help="Visualization mode (default: reify)"
    )
    p1.add_argument(
    ...
    p1.add_argument(
        "--empty",
        action="store_true",
        help="Embed an empty graph (listen-only) instead of dumping current DB",
    )
    p1.add_argument(
        "--cdc-ws-url",
        help="Enable live CDC and connect to this WebSocket URL (e.g. ws://127.0.0.1:8787/changes/ws)",
    )
    p1.set_defaults(func=_cmd_one)

    p2 = sub.add_parser(
        "bundle", help="Dump paired bundles (KG + Conversation) into one folder"
    )
    p2.add_argument(
        "--empty",
        action="store_true",
    ...
    p2.add_argument(
        "--kg-persist-dir",
        required=False,
        help="Chroma persist dir for knowledge graph, not required when dump empty CDC receiver",
    )
    p2.add_argument(
        "--conversation-persist-dir",
        required=False,
        help="Chroma persist dir for conversation graph, not required when dump empty CDC receiver",
    )
    p2.add_argument(
    ...
    p2.add_argument(
        "--conversation-out",
        default="conversation.bundle.html",
        help="Conversation bundle filename within out-dir",
    )
    p2.add_argument(
        "--cdc-ws-url",
        help="Enable live CDC in all bundles and connect to this WebSocket URL",
    )
...
# dump app data
# python kogwistar\utils\kge_debug_dump.py bundle \
#    bundle
...
#    --template kogwistar\templates\d3.html \
