...
def test_seed_auth_data(auth_db):
    ...
    identity = auth_db.query(ExternalIdentity).filter(ExternalIdentity.subject == "alice-sub").first()
...
def test_oidc_flow_integration(monkeypatch, auth_engine, auth_db):
    ...
    with patch("graph_knowledge_engine.server.auth.router._get_auth_mode", return_value="oidc"):
        with patch("graph_knowledge_engine.server.auth.router.get_oidc_client") as mock_oidc_factory:
            mock_oidc = AsyncMock()
            ...
            identity = auth_db.query(ExternalIdentity).filter(ExternalIdentity.subject == "oidc-sub").first()
            assert identity is not None
