...
def test_de_alias_ids_in_result_session_alias(monkeypatch):
    ...
    n1 = Node(label="A", type="entity", summary="a", references=[ReferenceSession(collection_page_url="c", document_page_url=f"document/{doc.id}", start_page=1,end_page=1,start_char=0,end_char=1)])
    n2 = Node(label="B", type="entity", summary="b", references=[ReferenceSession(collection_page_url="c", document_page_url=f"document/{doc.id}", start_page=1,end_page=1,start_char=2,end_char=3)])
    ...
    e = Edge(label="A->B", type="relationship", summary="ab", relation="rel", source_ids=[n1.id], target_ids=[n2.id],
             references=[ReferenceSession(collection_page_url="c", document_page_url=f"document/{doc.id}", start_page=1,end_page=1,start_char=0,end_char=3)])
    ...
    parsed = LLMGraphExtraction(
        nodes=[LLMNode(id=book.real_to_alias[n1.id], label="A", type="entity", summary="a",
                       references=[ReferenceSession(collection_page_url="c", document_page_url=f"document/{doc.id}", start_page=1,end_page=1,start_char=0,end_char=1, verification=MentionVerification(method="heuristic", is_verified=False))])],
        edges=[LLMEdge(id=book.real_to_alias[e.id], label="A->B", type="relationship", summary="ab",
                       ...
                       references=[ReferenceSession(collection_page_url="c", document_page_url=f"document/{doc.id}", start_page=1,end_page=1,start_char=0,end_char=3, verification=MentionVerification(method="heuristic", is_verified=False))])]
    )
...
def test_commit_merge_creates_same_as_and_endpoints():
    ...
    ref = ReferenceSession(collection_page_url="c", document_page_url=f"document/{doc.id}", start_page=1, end_page=1, start_char=0, end_char=5)
