...
def test_default_embedder_autoruns(tmp_path):
    ...
    n = Node(
        ...
        mentions=[kg_grounding(doc.id, start_char=0, end_char=10, excerpt="Chlorophyll")],
    )
    eng.write.add_node(n, doc_id=doc.id)  # embeddings=None -> auto-embed via DefaultEmbeddingFunction
