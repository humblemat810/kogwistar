...
def _patch_single_step_design(monkeypatch, *, workflow_id: str, op: str) -> None:
    ...
    monkeypatch.setattr("kogwistar.runtime.async_runtime.validate_workflow_design", _validate)
...
def test_async_runtime_resolver_send_lane_message_reaches_sync_mirror(
    ...
    async def _send(ctx):
        sent = ctx.send_lane_message(
            ...
            correlation_id="corr:runtime-async",
        )
        return RunSuccess(
            conversation_node_id=None,
    ...
    assert rows[0].correlation_id == "corr:runtime-async"
...
def test_async_runtime_resolver_send_lane_message_appends_run_registry_event(
    tmp_path, monkeypatch
):
    ...
    async def _send(ctx):
        sent = ctx.send_lane_message(
            ...
            run_id=ctx.run_id,
            step_id=str(ctx.step_seq),
            correlation_id="corr:runtime-async-registry",
        )
        return RunSuccess(
            conversation_node_id=None,
            state_update=[("u", {"lane_message_id": sent.message_id})],
