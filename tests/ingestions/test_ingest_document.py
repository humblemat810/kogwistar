...
def test_ingest_documentS_with_llm(engine: GraphKnowledgeEngine) -> None:
    ...
    instruction_for_node_edge_contents_parsing_inclusion = (
        "Extract knowledge as nodes and edges. Extract node as entity, edge as relationship. "
    ...
    for idx, doc_content in enumerate(doc_content_list):
        ...
        def _extract_only(
            ...
            raw_with_parsed = {}
            retry = 3
            while (raw_with_parsed.get("parsed") is None) or (
                raw_with_parsed.get("error") is not None
            ):
                raw_with_parsed = engine.extract_graph_with_llm(
                    content=content,
                    doc_type=doc.type,
        ...
        extracted = _extract_only(
            doc.content, instruction_for_node_edge_contents_parsing_inclusion
        )
        parsed = LLMGraphExtraction["llm"].model_validate(
            extracted["parsed"],
            context={"insertion_method": "test_case/test_ingest_documentS_with_llm"},
        )

        # persist deterministically (choose replace/append/skip-if-exists)
        out = engine.persist_graph_extraction(
            document=doc, parsed=parsed, mode="replace"
        )
        outcome.append(out)
    outcome
    pass
