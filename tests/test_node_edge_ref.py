...
def test_node_refs_indexing(engine):
    ...
    n = Node(label="A", type="entity", summary="s", mentions=[ref_llm, ref_manual])
...
def test_edge_refs_indexing(engine):
    ...
    a = Node(label="A", type="entity", summary="s", mentions=[ref_llm]); engine.add_node(a, doc_id=doc.id)
    b = Node(label="B", type="entity", summary="s", mentions=[ref_manual]); engine.add_node(b, doc_id=doc.id)

    e = Edge(
        label="A->B", type="relationship", summary="rel", relation="related",
        source_ids=[a.id], target_ids=[b.id], source_edge_ids=[], target_edge_ids=[],
        mentions=[ref_llm, ref_manual],
    )
