...
def test_node_refs_indexing(engine):
    ...
    ref_llm = ReferenceSession(
        ...
        start_page=1, end_page=1, start_char=0, end_char=5, doc_id=doc.id, insertion_method = "graph_extractor",
        verification=MentionVerification(method="llm", is_verified=True, score=0.9),
    )
    ref_manual = ReferenceSession(
        collection_page_url="c", document_page_url=f"document/{doc.id}",
        start_page=2, end_page=2, start_char=0, end_char=5, doc_id=doc.id, insertion_method = "graph_extractor",
        verification=MentionVerification(method="manual", is_verified=True, score=1.0),
    )
    ...
    assert n.id in engine.nodes_by_doc(doc.id, insertion_method="llm")
    assert n.id in engine.nodes_by_doc(doc.id, insertion_method="manual")
...
def test_edge_refs_indexing(engine):
    ...
    assert e.id in engine.edges_by_doc(doc.id, insertion_method="llm")
    assert e.id in engine.edges_by_doc(doc.id, insertion_method="manual")