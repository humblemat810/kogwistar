...
def _count_next_turn_edges(
    conversation_engine, *, conversation_id: str, src: str, tgt: str
) -> int:
...
def test_next_turn_duplicate_is_idempotent_in_add_edge(phase1_engine_pair):
    ...
    assert (
        _count_next_turn_edges(
            conversation_engine, conversation_id=conversation_id, src=t1.id, tgt=t2.id
        )
        == 1
    )
...
def test_dependency_freeze_rejects_new_incoming_into_used_node_without_scanning_all_edges(
    ...
    def _fail_get_edges(*args, **kwargs):
        raise AssertionError(
            "get_edges() should not be called for dependency-freeze validation"
        )
