...
def test_summary_creates_context_snapshot_before_llm_call(backend_kind, tmp_path, sa_engine, pg_schema, monkeypatch):
    ...
    for idx, txt in enumerate(["hi", "tell me X", "ok thanks"], start=0):
        nid = get_id_for_conversation_turn(ConversationNode.id_kind, user_id, conversation_id, txt, str(idx), "user", "conversation_turn", "True")
        n = ConversationNode(
            id=nid,
            label=f"Turn {idx}",
            type="entity",
    ...
    assert snap.metadata.get("stage") in (None, "summary")  # stage may be stored in metadata; tolerate older layouts
    ...
    assert all("cost.char_count" not in (t.metadata or {}) for t in turns), "accounting must not be stored on turn nodes"
...
def test_answer_flow_creates_context_snapshots_and_edges(backend_kind, tmp_path, sa_engine, pg_schema, monkeypatch):
    ...
    nid = get_id_for_conversation_turn(ConversationNode.id_kind, user_id, conversation_id, "What is chlorophyll?", "0", "user", "conversation_turn", "True")
    conv.add_node(
        ConversationNode(
            id=nid,
            label="Turn 0",
    ...
    out = agent.answer(conversation_id=conversation_id, user_id=user_id, prev_turn_meta_summary=prev)
