...
def test_conversation_flow(
    ...
    conversation_engine._oplog = OplogWriter(
        Path(".") / "bundle" / "conv_changes.jsonl", fsync=False
    )
                ...
                spans=[
                    Span(
                        ...
                        verification=MentionVerification(
                            ...
                            is_verified=True,
                            score=1.0,
                            notes="test:verbatim user/system input",
                        ),
    ...
    conv_id, start_node_id_returned = svc.create_conversation(
        user_id, conv_id, start_node_id
    )
    assert conv_id
    assert start_node_id_returned == start_node_id
    # Check start node
    start_nodes_dict = conversation_engine.backend.node_get(
    ...
    candiate_filtering_callback_cached = cached(
        memory, candiate_filtering_callback, ignore=["llm_tasks"]
    )
    ...
    ref_nodes = conversation_engine.backend.node_get(
    ...
    referred_ids = [
        json.loads(i)["properties"]["refers_to_id"] for i in ref_nodes["documents"]
    ]
    ...
    user_conv_chain_length = conversation_orchestrator.get_chain_length(
    ...
    for i in range(i_start, i_end):
        assert i == conversation_orchestrator.get_chain_length(
            user_id=user_id, conversation_id=conv_id
        )
    ...
    # Check Summary Node
    summaries = conversation_engine.get_nodes(
        where={"entity_type": "conversation_summary"}
    )
