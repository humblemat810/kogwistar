...
class _BackendShim:
    ...
    def __init__(
        self, node_collection: FakeCollection, edge_collection: FakeCollection
    ):
...
def engines():
    ...
    kg.node_collection.add(
        "A", metadata={"label": "Foo", "summary": "Foo is a thing.", "type": "concept"}
    )
    kg.node_collection.add(
        "B", metadata={"label": "Bar", "summary": "Bar is related.", "type": "concept"}
    )
    kg.node_collection.add(
        "C", metadata={"label": "Baz", "summary": "Baz is extra.", "type": "concept"}
    )
...
def test_agent_answer_creates_run_anchor_projects_used_evidence(monkeypatch, engines):
    ...
    def fake_get_conversation_view(
        self,
        *,
        conversation_id,
        user_id=None,
    ...
    gen_eid = edge_id(
        ...
        rel="generated",
        src=out["run_node_id"],
        dst=out["assistant_turn_node_id"],
    )
...
def test_projection_is_idempotent(engines):
    ...
    run_node_id = agent._ensure_run_anchor(
        conversation_id=conversation_id, run_id="run_test"
    )
...
def test_agent_with_real_llm_cached(monkeypatch, engine, conversation_engine):
    ...
    def select_used_evidence_cached(query: str, candidates: list[dict]) -> list[str]:
        messages = [
            {
                "role": "system",
                "content": "Select the IDs of evidence items that are truly USED.",
            },
            {
                "role": "user",
                "content": json.dumps(
                    {"query": query, "candidates": candidates}, sort_keys=True
                ),
            },
        ]
    ...
    def generate_answer_with_citations_cached(
        ...
        *,
        ...
        messages = [
            {
                "role": "system",
                "content": "Answer the user using ONLY the provided evidence.",
            },
            {
                "role": "user",
                "content": json.dumps(
                    {"query": question, "used": used_node_ids}, sort_keys=True
                ),
            },
        ]
        payload = _llm_cache_key(
            model_name,
            temperature,
            messages,
            out_model_schema,
    ...
    monkeypatch.setattr(
        agent, "_generate_answer_with_citations", generate_answer_with_citations_cached
    )
    ...
    conv_svc.add_conversation_turn(
        user_id,
        conv_id,
        role="user",
        turn_id="user_turn1",
